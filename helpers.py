...
def get_active_project(
    ...
    if docs_count > 0:
        ...
        actives_count = db.projects.count_documents(
        ...
        if actives_count == 1:
            if include_tasks:
                ...
                project = db.projects.find_one(
                    {"pm_tg_id": pm_tg_id, "active": True}, {"tasks": 0}
                )
        ...
        try:
            ...
            if team_member and type(team_member) is dict:
                ...
                if len(people) > 0:
                    ...
                    people = (
                        f"{people}@{team_member['tg_username']} ({team_member['name']})"
                    )
    ...
    if context.job_queue:
        ...
        if job:
            try:
                ...
                hour = int(
                    str(job.trigger.fields[job.trigger.FIELD_NAMES.index("hour")])
                )
                minute = int(
                    str(job.trigger.fields[job.trigger.FIELD_NAMES.index("minute")])
                )
                days_preset = str(
                    job.trigger.fields[job.trigger.FIELD_NAMES.index("day_of_week")]
                )
            except (LookupError, AttributeError, ValueError) as e:
                preset = {}
                logger.error(
                    f"Error occured while getting preset for job_id '{job_id}': {e}"
                )
            else:
...
def get_keyboard_and_msg(
    ...
    if (
        "title" in project.keys()
        and "settings" in project.keys()
        and "reminders" in project.keys()
        and project["title"]
            ...
            case 1:
                match branch:
                    case "notifications":
                        if is_db(db):
                            try:
                                pm_settings = db.staff.find_one(
                                    {"tg_id": str(user_id)}, {"settings": 1, "_id": 0}
                                )
                            except PyMongoError as e:
                                logger.error(f"Error using DB: {e}")
                            else:
                        ...
                        if is_db(db):
                            ...
                            for project in projects:
                                keyboard.append(
                                    [
                                        InlineKeyboardButton(
                                            f"Rename: '{project['title']}'",
                                            callback_data=f"rename_{project['_id']}",
                                        )
                                    ]
                                )
                                ...
                                if project["active"] is False:
                                    row = [
                                        InlineKeyboardButton(
                                            ...
                                            callback_data=f"activate_{project['_id']}",
                                        ),
                                        InlineKeyboardButton(
                                            f"Delete '{project['title']}'",
                                            callback_data=f"delete_{project['_id']}",
                                        ),
                                    ]
                                    keyboard.append(row)
                            keyboard.extend(
                    ...
                    case "reminders":
                        ...
                        keyboard = [
                            [
                                InlineKeyboardButton(
                                    "Reminder on day before",
                                    callback_data="day_before_update",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "Everyday morning reminder",
                                    callback_data="morning_update",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "Friday reminder of project files update",
                                ...
                                for member in team:
                                    if member["tg_id"] and user_id != member["tg_id"]:
                                        keyboard.append(
                                            [
                                                InlineKeyboardButton(
                                                    f"{member['name']} (@{member['tg_username']})",
                                                    callback_data=member["tg_id"],
                                                )
                                            ]
                                        )
                            if not keyboard:
...
def get_message_and_button_for_task(
    ...
    if msg:
        button = InlineKeyboardButton(
            "Mark as completed🏁", callback_data=f"task_{project_id}_{task['id']}"
        )
...
def get_project_by_title(db: Database, pm_tg_id: str, title: str) -> dict:
    ...
    # Get project from DB
    project = db.projects.find_one({"pm_tg_id": pm_tg_id, "title": title})
...
def get_projects_and_pms_for_user(user_oid: ObjectId | str, db: Database) -> str:
    ...
    try:
        projects = list(
            db.projects.find(
                {"tasks.actioners": {"$elemMatch": {"actioner_id": oid}}},
        ...
        if projects:
            for project in projects:
                ...
                if (
                    ...
                    if pm_un:
                        projects_and_pms = (
                            projects_and_pms + f"'{project['title']}' (@{pm_un})"
                        )
                    else:
        ...
        for task in project["tasks"]:
            ...
            if "actioners" in task.keys() and task["actioners"]:
                for actioner in task["actioners"]:
                    ...
                    if not any(
                        str(actioner["actioner_id"]) in str(member["_id"])
                        for member in team
                    ):
                        try:
                            result = db.staff.find_one(
                                {"_id": ObjectId(actioner["actioner_id"])}
                            )
                        except PyMongoError as e:
                            logger.error(f"Error using DB: {e}")
                        else:
...
def get_status_on_project(project: dict, user_oid: ObjectId | str, db: Database) -> str:
    ...
    if pm_username:
        ...
        logger.error(
            f"PM (with tg_id: {project['pm_tg_id']}) was not found in db.staff!"
        )
    ...
    if actioner_username:
        ...
        for task in project["tasks"]:
            if (
                ...
                any(
                    str(user_oid) in str(doer["actioner_id"])
                    for doer in task["actioners"]
                )
            ):
                # If delta_start <0 task not started, otherwise already started
            ...
            if (
                inform_of_milestone
                and task["milestone"]
                and date.today() == date.fromisoformat(task["enddate"])
            ):
                msg = (
                    msg
